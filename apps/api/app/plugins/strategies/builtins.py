...
from app.plugins.strategies.base import BaseStrategyPlugin, StrategyDecision


class EmaRsiVolumeStrategy(BaseStrategyPlugin):
    name = "EMA_RSI_VOLUME"
    description = "EMA trend plus RSI and volume confirmation."
...
class MacdBreakoutStrategy(BaseStrategyPlugin):
    name = "MACD_BREAKOUT"
    description = "MACD plus breakout confirmation."
    supports_backtest = True
...
class SupportResistanceBreakoutStrategy(BaseStrategyPlugin):
    name = "SUPPORT_RESISTANCE_BREAKOUT"
    description = "Support and resistance breakout workflow."
    supports_backtest = True
...
class MeanReversionRsiStrategy(BaseStrategyPlugin):
    name = "MEAN_REVERSION_RSI"
    description = "RSI mean reversion near support and resistance."
    supports_backtest = True
...
class GridSimpleStrategy(BaseStrategyPlugin):
    name = "GRID_SIMPLE"
    description = "Grid scaffold for future automation."
    supports_backtest = False

    def analyze(self, config: dict, market_snapshot: dict, account_snapshot: dict, open_positions: list[dict]) -> StrategyDecision:
        return StrategyDecision("WATCH", 45, "Grid Simple is scaffolded for dry-run structure.", float(Decimal(market_snapshot["analysis_close"])), 1.0, 1.0, {"phase": "phase_5"})


class DcaSimpleStrategy(BaseStrategyPlugin):
    name = "DCA_SIMPLE"
    description = "Budgeted step-entry DCA scaffold."
    supports_backtest = False

    def analyze(self, config: dict, market_snapshot: dict, account_snapshot: dict, open_positions: list[dict]) -> StrategyDecision:
        price = Decimal(market_snapshot["analysis_close"])
        return StrategyDecision("WATCH", 52, "DCA Simple is waiting for configured step drop.", float(price), 2.0, 1.5, {"max_steps": config.get("max_steps", 3), "step_drop_pct": config.get("step_drop_pct", 1.5)})


class AiTechnicalGuardStrategy(BaseStrategyPlugin):
    name = "AI_TECHNICAL_GUARD"
    description = "Delegates to AI engine plus technical guard at runtime."
    supports_backtest = True
